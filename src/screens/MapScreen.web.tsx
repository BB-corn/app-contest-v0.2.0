...
function injectCSS() {
  ...
  s.textContent = `
    ...
    /* ── 导航定位点（平滑圆点 + 脉冲） ── */
    .nav-dot{background:transparent!important;border:none!important}
    .nav-dot-wrap{width:40px;height:40px;display:flex;align-items:center;justify-content:center}
    .nav-dot-accuracy{
      position:absolute;width:120px;height:120px;border-radius:50%;background:rgba(212,160,23,0.06);
      transform:translate(-40px,-40px);pointer-events:none;animation:jm-pulse 3s ease-in-out infinite;
    }
    .nav-dot-inner{
      width:22px;height:22px;border-radius:50%;background:linear-gradient(160deg,#2d8ee6,#1b6fd1);
      border:3px solid rgba(255,255,255,0.95);box-shadow:0 6px 18px rgba(27,111,209,0.18);
    }
    .nav-dot-outer{
      position:absolute;width:40px;height:40px;border-radius:50%;top:0;left:0;pointer-events:none;
      animation:nav-dot-pulse 1.6s ease-out infinite;opacity:.9
    }
  ...
  const segFetches = coords.slice(0, -1).map(async (origin, i) => {
    ...
    try {
      ...
      (data.route.paths[0].steps as { polyline: string }[]).forEach(
        (step, si) => {
          const sp = parseAmapPolyline(step.polyline);
...
export function MapScreen() {
    ...
    loadLeaflet()
      .then((L) => {
        ...
        landmarks.forEach((l) => {
          ...
          const [gcjLat, gcjLng] = wgs84ToGcj02(l.latitude, l.longitude);
  ...
  const clearRouteLayers = useCallback(() => {
    // No-op: snake/route layer animation removed.
  }, []);
  ...
  const flyToRouteBounds = useCallback((route: Route) => {
    ...
    if (route.coordinates.length === 1) {
      const [gcjLat, gcjLng] = wgs84ToGcj02(route.coordinates[0][0], route.coordinates[0][1]);
      map.flyTo([gcjLat, gcjLng], 14, { duration: 0.8 });
      return;
    }
    const gcjCoords = route.coordinates.map(([lat, lng]) => wgs84ToGcj02(lat, lng));
    const bounds = L.latLngBounds(gcjCoords);
  ...
  const startNavigation = useCallback(
    async (route: Route) => {
      ...
      if (!roadCoords || roadCoords.length < 20) {
        ...
        const gcjCoords = route.coordinates.map(([lat, lng]) =>
          wgs84ToGcj02(lat, lng) as [number, number]
        );
        roadCoords = smoothPathFallback(gcjCoords, 60);
      }
        ...
        icon: L.divIcon({
          ...
          html: `
            <div class="nav-dot-wrap">
              <div class="nav-dot-outer" style="border:2px solid rgba(45,142,230,0.14);"></div>
      ...
      const duration = Math.max(4000, Math.min(14000, total * 22));
  ...
  const flyTo = useCallback(
    (l: Landmark) => {
      ...
      setSelected(l);
      addRecent(l.id);
      // activeRouteId removed
      clearRouteLayers();
      if (mapRef.current) {
        const [gcjLat, gcjLng] = wgs84ToGcj02(l.latitude, l.longitude);
        mapRef.current.flyTo([gcjLat, gcjLng], 15, {
          duration: 0.8
        });
      }
    },
